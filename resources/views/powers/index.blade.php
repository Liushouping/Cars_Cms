...
<div class="flex justify-center items-center text-white w-full bg-gradient-to-r from-blue-900 to-blue-500"  style="height:150px;">
  <h2 class="text-4xl font-bold tracking-wider">:: {{ __('性能車款') }}</h2>
</div>
@stop
        ...
        <div class="hero">
            ...
            <div class="flex justify-center items-center mb-12">
                <div class="w-full rounded-lg">
                    <form action="{{ route('power' , app()->getLocale() ) }}" class="search-form">
                    ...
                    @forelse( $powers as $power )
                        <article class="bg-white group relative rounded-lg overflow-hidden shadow hover:shadow-md transition-shadow duration-300 ease-in-out" data-aos="fade-up" data-aos-delay="300">
                            <div class="relative w-full">
                                <img src="{{$power->image_url}}"
                                    class="w-full h-full object-center object-cover">
                                <p class="absolute top-0 bg-red-600 text-white font-semibold py-1 px-3 rounded-br-lg rounded-tl-lg">${{$power->price}}</p>
                            </div>
                            <div class="px-3 py-4">
                                ...
                                {{ __('車型') }}：{{$power->model}}<br><br>
                                {{ __('引擎') }}：{{$power->engine}}<br><br>
                                {{ __('排氣量') }}：{{$power->​exhaust}}<br><br>
                                {{ __('最大馬力') }}：{{$power->max_horsepower}}<br><br>
                                {{ __('最大扭力') }}：{{$power->​max_torque}}
                                </div>
